...
function WorkspacePageInner() {
  ...
  // Messages forwarded from the main ChatPanel, used to populate the Whiteboard.
  const [chatMessages, setChatMessages] = useState<UIMessage[]>([]);
  // Images attached in the chat panel, shown on the whiteboard.
  const [chatImages, setChatImages] = useState<Array<{ id: string; url: string; label: string }>>([]);

  const handleChatImageAttached = useCallback((id: string, serverUrl: string, name: string) => {
    setChatImages((prev) => {
      if (prev.some((img) => img.id === id)) return prev;
      return [...prev, { id, url: serverUrl, label: name }];
    });
  }, []);

  const handleChatImageRemoved = useCallback((_id: string) => {
    // Don't remove from whiteboard — let the user dismiss it there.
    // The image stays visible on the whiteboard even after chat attachment is removed.
  }, []);

  const handleWhiteboardImageDoubleClick = useCallback((content: string, label: string) => {
    void chatRef.current?.addWhiteboardImage?.(content, label);
  }, []);
  ...
  // Fetch chat sessions (with timeout to avoid indefinite loading)
  const fetchSessions = useCallback(async () => {
    setSessionsLoading(true);
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10_000);
    try {
      const res = await fetch("/api/web-sessions", { signal: controller.signal });
      clearTimeout(timeoutId);
      const data = await res.json();
      setSessions(data.sessions ?? []);
    } catch {
      // ignore (network error, timeout, etc.)
      setSessions([]);
    } finally {
      clearTimeout(timeoutId);
  ...
  const handleGenerateFromEntry = useCallback((objectName: string, entryId: string) => {
    const msg = `Generate images for ${objectName} entry ${entryId}. ` +
      `Look up the entry in the shot list and use both the Description and Notes fields (plus Generation Prompt if present) to build the prompt. Follow the production workflow for image generation.`;
    setActivePath(null);
    setContent({ kind: "none" });
    handleCloseEntry();
    requestAnimationFrame(() => {
      void chatRef.current?.sendNewMessage(msg);
    });
  }, [handleCloseEntry]);
    ...
    >
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden" style={{ background: "var(--color-main-bg)" }}>
        ...
        <div className="flex-1 flex min-h-0">
          {showMainChat ? (
            ...
            <>
              {isMobile ? (
                /* Mobile: Whiteboard on top + ChatPanel below + sessions drawer */
                <>
                  <div className="flex-1 flex flex-col min-w-0 min-h-0" style={{ background: "var(--color-main-bg)" }}>
                    <div className="shrink-0 flex h-[min(38vh,280px)] min-h-[140px] flex-col border-b overflow-hidden" style={{ borderColor: "var(--color-border)" }}>
                      <Whiteboard messages={chatMessages} externalImages={chatImages} onImageDoubleClick={handleWhiteboardImageDoubleClick} workspaceId={workspaceName} />
                    </div>
                    <div className="flex-1 flex flex-col min-h-0 min-w-0">
                      <ChatPanel
                        ...
                        onImageAttached={handleChatImageAttached}
                        onImageRemoved={handleChatImageRemoved}
                      />
                    </div>
                  </div>
                  {chatSessionsOpen && (
                ...
                /* Desktop: Whiteboard in main area + ChatPanel in right sidebar */
                <>
                  {/* Whiteboard — visual canvas that mirrors what the AI is building */}
                  <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                    <Whiteboard messages={chatMessages} externalImages={chatImages} onImageDoubleClick={handleWhiteboardImageDoubleClick} workspaceId={workspaceName} />
                  </div>
                  {/* Right sidebar: ChatPanel (or BrowserPreview / ChatSidebarPreview) */}
      ...
      {entryModal && (
        <EntryDetailModal
          ...
          onGenerate={() => handleGenerateFromEntry(entryModal.objectName, entryModal.entryId)}
        />
      )}
    </div>
  );
...
function ObjectView({
  ...
  const handleGenerateEntry = useCallback((entry: Record<string, unknown>) => {
    if (!onSendToChat) return;
    const findField = (keywords: string[]) => {
      for (const kw of keywords) {
        const match = data.fields.find((f) => f.name.toLowerCase().includes(kw));
        if (match) {
          const val = entry[match.name];
          if (val != null && String(val).trim()) return String(val).trim();
        }
      }
      return "";
    };
    const genPrompt = findField(["generation prompt", "generation_prompt", "prompt"]);
    const description = findField(["description", "desc"]);
    const notes = findField(["notes", "note"]);
    const promptParts = [genPrompt, description, notes].filter(Boolean);
    const prompt = promptParts.length > 0 ? promptParts.join(". ") : "";
    const shotNum = findField(["shot #", "shot_number", "shot"]);
    const sceneNum = findField(["scene #", "scene_number", "scene"]);

    let msg = "Generate images for";
    if (sceneNum) msg += ` scene ${sceneNum}`;
    if (shotNum) msg += ` shot ${shotNum}`;
    if (!sceneNum && !shotNum) msg += ` this ${data.object.name} entry`;
    if (prompt) msg += `. Use the following prompt: ${prompt}`;
    msg += ". Follow the production workflow for image generation.";

    onSendToChat(msg);
  }, [onSendToChat, data.fields, data.object.name]);
    ...
    <div className="p-6">
      ...
      {currentViewType === "table" && (
        <ObjectTable
          ...
          onGenerateEntry={onSendToChat ? handleGenerateEntry : undefined}
        />
      )}
