...
import { ChatSessionsSidebar } from "./workspace/chat-sessions-sidebar";
...
export type ChatPanelHandle = {
	...
	/** Fetch an image from the given URL/data-URL and attach it to the chat. */
	addWhiteboardImage?: (content: string, label: string) => Promise<void>;
};
...
type ChatPanelProps = {
	...
	onBack?: () => void;
	/** Called whenever the message list changes (e.g. to feed a Whiteboard). */
	onMessagesChange?: (messages: UIMessage[]) => void;
	/** Called when an image attachment finishes uploading (server URL ready). */
	onImageAttached?: (id: string, serverUrl: string, name: string) => void;
	/** Called when an image attachment is removed from the chat. */
...
export const ChatPanel = forwardRef<ChatPanelHandle, ChatPanelProps>(
	function ChatPanelInner(
		...
		const [allSessions, setAllSessions] = useState<Array<{ id: string; title: string; createdAt: number; updatedAt: number; messageCount: number }>>([]);
		...
		// Refresh sessions list when sidebar opens or sessions change
		useEffect(() => {
			if (showHistorySidebar) {
				void fetchAllSessions();
			}
		}, [showHistorySidebar, fetchAllSessions]);
		...
		// Forward messages to parent (e.g. Whiteboard) whenever they change.
		useEffect(() => {
		...
		// Ref so addWhiteboardImage (in useImperativeHandle) can call uploadAndAttachNativeFiles
		// even though it's declared later in the file.
		const uploadAndAttachNativeFilesRef = useRef<(files: FileList | File[]) => void>(() => {});
	...
	useImperativeHandle(
		...
		newSession: async () => { handleNewSession(); },
			...
			addWhiteboardImage: async (content: string, label: string) => {
				try {
					// Fetch the image (works for data URLs, blob URLs, and server URLs)
					const res = await fetch(content);
					const blob = await res.blob();
					const ext = blob.type.split("/")[1] || "png";
					const safeName = label.replace(/[^a-z0-9._-]/gi, "_") || "whiteboard-image";
					const fileName = safeName.endsWith(`.${ext}`) ? safeName : `${safeName}.${ext}`;
					const file = new File([blob], fileName, { type: blob.type });
					uploadAndAttachNativeFilesRef.current([file]);
				} catch {
					// Silently fail if the image can't be fetched (e.g. CORS)
				}
			},
		}),
	...
	const removeAttachment = useCallback((id: string) => {
		setAttachedFiles((prev) => {
			...
			if (removed && getFileCategory(removed.name) === "image") {
				onImageRemoved?.(id);
			}
				...
				fetch("/api/workspace/upload", {
					...
					.then((json: { ok?: boolean; path?: string } | null) => {
						if (json?.ok && json.path) {
							...
							// Notify parent so the whiteboard can show this image
							if (isImage && onImageAttached) {
								const serverUrl = `/api/workspace/raw-file?path=${encodeURIComponent(json.path!)}`;
								onImageAttached(placeholderId, serverUrl, file.name);
							}
						} else {
	...
	// Keep ref in sync so useImperativeHandle's addWhiteboardImage can call it.
	uploadAndAttachNativeFilesRef.current = uploadAndAttachNativeFiles;
	...
	// ── Status label ──
		...
		return (
			...
			>
				...
				{(error || streamError) && (
					<div
						className="px-3 py-2 flex flex-col gap-1.5 sticky bottom-[72px] z-10"
						style={{
					...
					>
						<div className="flex items-start gap-2">
							...
							<div className="min-w-0 flex-1">
								...
								{(error?.message?.includes("Gateway WebSocket") ?? streamError?.includes("Gateway WebSocket")) && (
									<p className="text-[11px] mt-1" style={{ color: "var(--color-text-muted)" }}>
										The OpenClaw gateway may not be running. Try: <code className="px-1 py-0.5 rounded" style={{ background: "var(--color-surface-hover)" }}>npx animclaw restart</code> or <code className="px-1 py-0.5 rounded" style={{ background: "var(--color-surface-hover)" }}>openclaw --profile animclaw gateway restart</code>
									</p>
								)}
							</div>
						</div>
			...
			{showHistorySidebar && (
				<ChatSessionsSidebar
					...
					onRenameSession={onRenameSession ? (sessionId, newTitle) => {
