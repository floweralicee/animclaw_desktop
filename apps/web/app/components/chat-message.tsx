...
export const ChatMessage = memo(function ChatMessage({ message, isStreaming, onSubagentClick, onFilePathClick, sessionId, userHtmlMap, onGenUIRespond }: { message: UIMessage; isStreaming?: boolean; onSubagentClick?: (task: string) => void; onFilePathClick?: FilePathClickHandler; sessionId?: string | null; userHtmlMap?: Map<string, string>; onGenUIRespond?: GenUIRespondHandler }) {
	...
	if (isUser) {
		...
		const tailPlain = attachmentInfo?.message ?? textContent;
		const bodyForUrls = stripContextPrefix(tailPlain);
		const videoEmbeds: Array<{ url: string; embed: NonNullable<ReturnType<typeof parseVideoEmbed>> }> = [];
		for (const u of extractHttpUrls(bodyForUrls)) {
			const embed = parseVideoEmbed(u);
			if (embed) {
				videoEmbeds.push({ url: u, embed });
			}
		}

		const urlOnlyVideos =
			videoEmbeds.length > 0 &&
			isOnlyUrls(attachmentInfo?.message ?? textContent);
		const urlStripRe = /https?:\/\/[^\s<>"'{}|\\^`[\]()]+/gi;
		const hasNonUrlText =
			stripContextPrefix(tailPlain)
				.replace(urlStripRe, "")
				.replace(/\s+/g, "")
				.length > 0;
		const showBubble =
			!urlOnlyVideos &&
			(attachmentInfo
				? !!(attachmentInfo.message.trim() || richHtml)
				: !!(hasNonUrlText || richHtml || videoEmbeds.length === 0));
		...
		if (videoEmbeds.length > 0) {
			return (
				<div className="flex flex-col items-end gap-2 py-2 w-full min-w-0">
					{attachmentInfo ? <AttachedFilesCard paths={attachmentInfo.paths} /> : null}
					<div className="flex flex-col items-end gap-2 w-full max-w-[min(92vw,480px)]">
						{videoEmbeds.map(({ url, embed }) => (
							<LinkPreviewCard key={url} url={url} embed={embed} variant="chat" className="w-full" />
						))}
					</div>
					{showBubble &&
						bubbleShell(
							attachmentInfo
								? "max-w-[80%] w-fit rounded-2xl rounded-br-sm px-3 py-2 text-sm leading-6 break-words chat-message-font"
								: "max-w-[80%] min-w-0 rounded-2xl rounded-br-sm px-3 py-2 text-sm leading-6 overflow-hidden break-words chat-message-font",
						)}
				</div>
			);
		}
