...
function ensureExperimentalWarningSuppressed(): boolean {
  ...
  if (
    isTruthyEnvValue(process.env.ANIMCLAW_NO_RESPAWN) ||
    isTruthyEnvValue(process.env.OPENCLAW_NO_RESPAWN)
  ) {
    return false;
  }
  if (
    isTruthyEnvValue(process.env.ANIMCLAW_NODE_OPTIONS_READY) ||
  ...
  process.env.ANIMCLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[animclaw] Failed to respawn CLI:",
