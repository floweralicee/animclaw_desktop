...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Electron desktop app: vendor-bin ships openclaw, duckdb, and a portable node.
  const vendorBin = process.env.ANIMCLAW_VENDOR_BIN;
  if (vendorBin && isDirectory(vendorBin)) {
    prepend.push(vendorBin);
    const vendorNodeBin = path.join(vendorBin, "node", "bin");
    if (isDirectory(vendorNodeBin)) {
      prepend.push(vendorNodeBin);
    }
  }
