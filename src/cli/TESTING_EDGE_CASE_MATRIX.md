...
| `profile.ts`            | root vs command-local bootstrap profile flag                             | `animclaw --profile X bootstrap` and `animclaw bootstrap --profile X` resolve to identical profile env  | `src/cli/profile.test.ts` (existing, expand)                      |
